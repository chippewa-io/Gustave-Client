...
class Services {
    lazy var gustaveServerURL: String? = {
            return UserDefaults(suiteName: "io.chippewa.gustave")?.string(forKey: "GustaveServerURL")
        }()
    ...
    func deleteProfileWithUDID(silent: Bool = false, secretGenerationCompleted: inout Bool) {
        guard let gustaveServerURL = gustaveServerURL else {
            print("Gustave Config File not readable... please ensure you've deployed the gustave config.mobileconfig file properly with Jamf Pro")
            return
        }
        ...
        var localSecretGenerationCompleted = secretGenerationCompleted  // create a local copy
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            if let error = error {
                ...
                if !silent {
                    ...
                    self.spinner.stop()
                    localSecretGenerationCompleted = true
                }
        ...
        secretGenerationCompleted = localSecretGenerationCompleted  // assign it back to the inout parameter
    }
    ...
    func generateSecret(spinner: Spinner, silent: Bool = false) {
        ...
        // Safely handle gustaveServerURL and url
        guard let gustaveServerURL = gustaveServerURL else {
            os_log(.error, log: log, "GustaveServerURL not found.")
            return
        }
        
        let urlString = "\(gustaveServerURL)/api/secret"
        guard let url = URL(string: urlString) else {
        ...
        let task = URLSession.shared.dataTask(with: request) { [self] data, response, error in
            ...
            if let _ = String(data: data, encoding: .utf8) {
                ...
                secretGenerationCompleted = true  // Set the secretGenerationCompleted flag to true
            } else {
    ...
    func updateComputer(id: String, value: String) {
        ...
        if let gustaveServerURL = gustaveServerURL {
